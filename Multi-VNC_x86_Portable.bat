...
ECHO.
ECHO Installo VNC . . .
PsExec.exe -nobanner \\%host% "C:\vncinstall\launcher.bat">nul

:configVNC
ECHO.
ECHO Arresto VNC . . .
PsExec.exe -nobanner \\%host% "C:\vncinstall\stop_uvnc_service.bat">nul
ECHO.
ECHO Copio il file di configurazione
...
timeout /t 2 >nul
ECHO Avvio VNC . . .
PsExec.exe -nobanner \\%host% "C:\vncinstall\start_uvnc_service.bat">nul
...
ECHO Password: ********
ECHO.
"%~dp0\jre\bin\java.exe" -jar "%~dp0\vnc\VncViewer.jar" PORT %porta% HOST %ip% PASSWORD %passwordjava%
ECHO.
ECHO Premere un tasto per tornare al menu' . . .
pause>nul
goto menu
